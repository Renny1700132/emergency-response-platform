...
20. 回填任务 commit、分支 push、PR、审核与合并结果及时间。G4 的功能、测试、RTM、日志和 `tasks.md = DONE` 在同一功能 PR 内完成；PR 创建后在同一分支补充 PR 编号和审核信息。合并提交哈希不要求写进自身 PR，直接以 Gitee PR 合并记录为证据并由 G4-11 汇总，禁止另建常规收口 PR。G4-03/04 的历史收口为规则生效前遗留的一次性例外，自 G4-05 起不得复制。
