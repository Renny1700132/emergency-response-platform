...
本项目为“博物馆智能运营中心——应急管理子系统”。第一关“立项竞标”已冻结，第二关需求与规格（M2）及原型走查已完成；当前正式启动第三关“设计与计划”。本轮仅完成 G3-00 的资料整理、治理切换与设计输入规划；概要设计、详细设计、数据库设计、接口/OpenAPI、ADR、测试计划和正式工程开发，均须在对应 G3 任务被明确启动后才可开展。

G1/G2 的任务、基线、评审和过程证据均为历史受控记录，不得删除或篡改。`prototype/` 仅可作为页面组织、交互和演示状态的参考，不得将其 localStorage、Mock 数据或 `prototypeStore` 直接视为第三关的正式数据库、接口或技术架构。第三关仍须继承已冻结的需求、关键数字、★属性、责任边界与验收条件。
