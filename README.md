...
本仓库用于《AI 辅助软件项目全生命周期开发·综合实习》课程项目。第一关“立项竞标”已完成收口与最终冻结；第二关“需求与规格”已通过 M2，并完成最小验证原型及走查。当前进入第三关“设计与计划”的启动整理阶段（G3-00）；正式设计和正式业务开发仅在相应任务明确启动后开展。
...
当前计划见 `tasks.md`。第三关后续正式成果将包括设计文档集、OpenAPI 契约、ADR、`plan.md`、工程任务看板、测试计划、`constitution.md`、设计挂接 RTM 与 M3 评审证据；在任务启动前不得把这些规划项误写为已完成成果。
