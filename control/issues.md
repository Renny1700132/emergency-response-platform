...
- 2026-09-20 A 整改：提交 `112d555` 删除正式件重复目录标题及候选状态提示；未改变业务正文、受控事实或数字。
- 2026-09-20 C 复验：OOXML 中目录标题精确计数为 1；Microsoft Word 16 只读重新分页并导出 8 页，8/8 页无裁切、重叠、乱码或表格越界，第 10 章目录标注与正文均为逻辑第 5 页；无障碍审计 high/medium/low 均为 0。用户明确确认第 5 页为正式正确口径，因此原记录中的 LibreOffice 第 6 页差异降为跨渲染器非阻断观察，不再构成导航错误。
- 内容保持性：与整改前相比仅删除“目 录”重复标题和“本文件为评审候选”状态提示；39 FR、34★、117 AC、29+10 范围、责任边界与 `AC-G2-FR-015-03` 均未变化。
- 复验说明：生成脚本本轮未同步修改；如后续重新生成正式件，须重新执行单目录标题与最终分页检查，避免版式问题复现。该维护提醒不阻断当前受控正式件准出。
- 关闭证据：`logs/reviews/2026-09-20_G3-11-C-compliance-review.md`、`logs/reviews/2026-09-20_G3-11-C-rereview-evidence.json`、A 整改提交 `112d555`。
- 状态：CLOSED / VERIFIED_BY_C（2026-09-20）
...
- 提出人：C
- 时间：2026-09-20
- 严重级别：MAJOR / BLOCKING_TO_G3-13_DONE
- 文件与位置：`docs/deliverables/22-配置管理计划.docx` 正文 9 张业务表；`logs/reviews/2026-09-20_G3-13-A-self-check.md`“结构与审计”。
- 问题：正式件除修订记录表外共有 9 张业务表，均未设置紧邻对象的独立表题和“表 X-Y”编号。A 自检以“章节上下文和相邻标题定位”作为通过依据，但 `docs/deliverables/SKILL.md` 的“图表编号与题注硬门禁”明确要求每张具有独立业务语义的表格必须有统一表题，禁止有表无表号。
- 影响：工作稿业务内容、39 FR、34★、117 AC、责任边界和 `AC-G2-FR-013-01` 追踪语义未受影响；但正式件的编号引用、审查定位和统一 DoD 未满足，G3-13 不能置 DONE。
- 最小修复：由主责 A 在工作稿对应表格前补齐统一表题，并同步正式件，为 9 张业务表按章内顺序设置紧邻表格且不跨页的 `表 X-Y 表名`；正文如引用表格，应使用对应编号。不得改变业务规则、数字、责任、Issue 状态或抽查结论。
- 关闭判据：工作稿与正式件的 9 张业务表均有唯一、连续、紧邻且可读的表题；无重复、跳号、题注与表格分页分离；重新执行全页渲染和题注计数，由 C 独立复验通过。
- 主责人：A；复核人：C。
- 证据：`logs/reviews/2026-09-20_G3-13-C-compliance-review.md`、`logs/reviews/2026-09-20_G3-13-C-review-evidence.json`。
- 状态：OPEN

#### A 整改响应（2026-09-20）

- 已将工作稿和正式件升级为 V0.2 评审候选，为 9 张业务表补齐 `表 1-1`、`表 2-1`、`表 3-1`、`表 4-1`、`表 5-1`、`表 6-1`、`表 7-1`、`表 9-1`、`表 10-1`。
- 机械复验：业务表题 9/9、紧邻表格 9/9、重复 0、章内跳号 0；Word 16 全页渲染 8/8 PASS，无题注与表格分页分离。
...
- 提出人：B
- 时间：2026-09-20
- 严重级别：MAJOR / BLOCKING_TO_G3-14_DONE
- 文件与位置：`docs/work/A_PM/risk_management_plan.md` §3.2、§4.1、§4.2；`docs/deliverables/23-风险管理计划与风险登记册v2.docx` 对应章节。
- 问题：计划在 §3.2 明确风险值 10—16 为“高”，§4.1 标题为“当前极高与高风险”，但 §4.2“中低风险与持续监测”仍放入 RR-03、RR-06、RR-07、RR-10、RR-11、RR-15 六项 12 分高风险。风险评分、章节分类和优先级展示相互冲突。
- 影响：风险本身、P/I 分值、责任人和应对内容基本完整，但高风险分散在“中低风险”栏目会误导状态报告、复核频率和资源优先级，不能支持可靠的风险排序与准出。
- 最小修复：由主责 A 保持各风险 ID、P/I、责任、触发、应对和关闭条件不变，将 10—16 分风险统一归入“高风险”口径；可将 §4.1 调整为“极高风险”，§4.2 调整为“高、中风险与持续监测”，或按评分阈值重新分表。同步目录、工作稿、正式件和自检后重新渲染。
- 关闭判据：全部 16 项风险的 P×I 与等级一致；章节标题/分组不再把高风险标为中低；风险 ID 和业务内容无丢失或重复；由 B 复验通过。
- 主责人：A；复核人：B。
- 状态：OPEN
- A 整改回应（2026-09-21）：已将 RR-03、RR-06、RR-07、RR-10、RR-11、RR-15 六项 12 分风险移入 §4.1“当前极高与高风险”，§4.2 仅保留 5—9 分中风险；16 项风险 ID、P/I、责任人、触发、应对、关闭条件及状态均保留。机械复核确认 16/16 项 P×I 与等级一致。待 B 独立复验，状态不变。
- B 复验（2026-09-21）：复跑机械审计确认 16 个风险 ID 无缺失或重复，P×I 与等级 16/16 一致；所有 12 分风险已归入 §4.1，§4.2 仅保留 5—9 分中风险且明确当前无低风险。原关闭判据满足。
- 状态：CLOSED / VERIFIED_BY_B
...
- 时间：2026-09-20
- 严重级别：MAJOR / BLOCKING_TO_G3-14_DONE
- 文件与位置：`docs/work/A_PM/risk_management_plan.md` 与 `docs/deliverables/23-风险管理计划与风险登记册v2.docx` 的 7 张业务表。
- 问题：正式件除修订记录布局表外共有 7 张业务表，工作稿和正式件均未设置独立的 `表 X-Y 表名` 题注。章节标题不能替代表题，不满足 `docs/deliverables/SKILL.md` 的图表编号与题注硬门禁。
- 影响：业务内容未因此丢失，但正式件的编号引用、审查定位和统一 DoD 不完整，G3-14 不能置 DONE。
- 最小修复：由主责 A 为 7 张业务表补齐唯一、连续、紧邻且不与表格分页分离的表题；同步工作稿和正式件，重新执行题注计数与全页渲染。不得改变风险内容、受控数字、AC 语义或开放 Issue 状态。
- 关闭判据：7/7 张业务表均有统一编号题注；无重复、跳号或题注—表格分页分离；全页渲染可读；由 B 复验通过。
- 主责人：A；复核人：B。
- 状态：OPEN
- A 整改回应（2026-09-21）：已为 7 张业务表补齐表 2-1、3-1、4-1、4-2、6-1、7-1、9-1 题注；工作稿与正式件同步，OOXML 检查确认 7/7 题注唯一且与表格相邻。Word 全页重渲染为 9 页并逐页检查可读；待 B 独立复验，状态不变。
- B 复验（2026-09-21）：工作稿和正式件均检出 7 个唯一题注，OOXML 题注—表格直接相邻 7/7；Word 16 重新渲染 9 页，未见题注与表格分页分离、裁切、重叠或表格越界。原关闭判据满足。
- 状态：CLOSED / VERIFIED_BY_B
...
- 时间：2026-09-21
- 严重级别：MAJOR / BLOCKING_TO_G3-14_DONE
- 文件与位置：`docs/deliverables/23-风险管理计划与风险登记册v2.docx` 第 3 页静态目录；正文逻辑第 5—6 页；`scripts/build_g3_14_risk_management_plan.py` 的 `replace_toc()`。
- 问题：整改后 Word 16 最终渲染为 9 页。静态目录把“6 AC-G2-FR-006-01 专项核对”标为正文第 4 页，实际位于正文逻辑第 5 页；把“9 主责自检与准出”标为第 5 页，实际位于正文逻辑第 6 页。A 的整改 QA 将 `toc_pagination_checked` 记为 true，但这两项与最终分页不一致。
- 影响：风险内容、评分、表题、AC 语义和开放 Issue 边界未受影响；但正式件目录无法准确定位，统一 DoD 的目录与最终分页检查未通过，G3-14 暂不能置 DONE。
- 最小修复：由 A 仅将静态目录第 6 章页码从 4 改为 5、第 9 章页码从 5 改为 6，并同步生成脚本；重新使用 Word 16 分页导出，逐项核对全部目录页码。不得改动正文、风险登记、表题或受控状态。
- 关闭判据：目录全部条目与 Word 16 最终正文逻辑页一致；正文内容快照除目录两处页码外无变化；全 9 页无视觉缺陷；由 B 复验通过。
- 主责人：A；复核人：B。
