...
正式 DOCX 中具有独立业务语义的表格、流程图、架构图、ER/UML/数据流图、甘特图、截图及其他插图，必须具有紧邻对象且格式统一的图题或表题。不得出现有表无表号、有图无图号、散乱手工题注、重复编号、跳号，或正文“如下图/表”无法唯一定位对象的情况。封面、修订记录等纯布局键值表仅在唯一 Reference 明确不编号时可维持该模板习惯。

编号、字体、位置和编号体系优先直接继承唯一 Reference；Reference 未给出明确规则时，表使用 `表 X-Y 表名`、图使用 `图 X-Y 图名`，其中 X 为章号、Y 为章内顺序。正文引用应使用存在的明确编号，例如“事件字段见表 5-2”“整体关系如图 2-1 所示”。

每次正式文档生成或修改后，QA 必须统计表格、图片/图形、图题、应编号表和表题数量；逐项检查题注覆盖、重复/跳号、题注位置、正文引用有效性与题注和对象是否分页分离。结果写入 Review/QA 记录。题注存在于 OOXML 不构成 PASS：必须完成 `DOCX → PDF/页面图渲染 → 逐页视觉检查 → 必要修复 → 重渲染`。
...
3. 物理复制 Reference DOCX 作为模板骨架；不得从空白文档重建。
...
`render_*` 目录及其 PDF、逐页 PNG/JPG/JPEG、contact sheet 等仅为内部视觉 QA 缓存，不是正式交付物或长期审计证据。检查结论、方法、页数、问题及修复结果应写入 Markdown/JSON；完成 QA 后必须删除这些渲染文件，除非当前任务明确要求长期保留并记录理由和范围。
