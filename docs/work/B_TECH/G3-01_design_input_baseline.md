...
| ISSUE-G3-01-002：信息发布、入侵、消防与 KN-064 挂接遗漏 | 会造成 G3-03/06 适配器遗漏及 G3-08/10 追踪断链 | 已补齐独立责任边界、EXT 端口清单和四系统联合验收链，提交 C 复验 | RESOLVED / PENDING_C_REVIEW；C 复验前阻断 G3-01 DONE |
...
- G3-06：为内部/外部端口形成接口说明和可校验 OpenAPI；EXT-PUBLISH、EXT-INTRUSION、EXT-ACCESS、EXT-FIRE、EXT-IOT、EXT-MIDDLE 分别定义语义和失败降级，未取得外部证据的字段明确 `【待人工确认】`。
...
| 是否识别开放阻断 | PASS；ISSUE-G3-01-001 保持 OPEN，ISSUE-G3-01-002 已整改并等待 C 复验 |
