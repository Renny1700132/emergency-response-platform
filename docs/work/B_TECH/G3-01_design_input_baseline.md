...
| 门禁/安防 | 终端、协议、授权、安全联锁及回执 | 授权后二次确认、单次下发、告警和人工降级 | 不绕过联锁，不自动重放控制指令 |
...
| MOD-INTEGRATION | 外部系统适配 | 视频、定位、消息、门禁、IoT/客流、安防、地图适配器及调用审计 | 005、006、020、026—029 | 全部甲方既有系统 |
...
3. MOD-INTEGRATION 隔离厂商协议，但不得包含事件核实、任务状态或门禁授权等业务决策。
...
A 可基于 MOD-PLAN—MOD-PLATFORM 和 ADC-001—006 拆分不超过 0.5 人日的原子任务。每项必须注明前置、责任人、输出、对应模块、具体 AC 和可执行验证；不得使用“设计完成”“功能正常”作为验证标准。接口证据补齐应作为明确依赖任务进入计划。

### 9.2 G3-03—G3-07 设计输入

- G3-03：按稳定模块 ID形成技术、数据、部署架构，给出模块交互、故障域和 NFR 预算。
- G3-04：落实主责数据、概念/逻辑/物理模型、索引、保存/归档与审计。
- G3-05：细化模块职责、类/组件、状态机、异常与补偿流程。
- G3-06：为内部/外部端口形成接口说明和可校验 OpenAPI；未取得外部证据的字段明确 `【待人工确认】`。
- G3-07：至少完成 ADC-001/002 ADR，并按 GB/T 25000.10 对非功能设计逐项标注质量特性。
- G3-08：C 使用稳定模块/设计 ID 回填 39 条 FR 和 117 条 AC 的设计挂接，禁止聚合项掩盖断链。
- G3-09：设计完成即锁定测试计划；测试级别、范围、策略、进度、准入准出和 AC 追踪均须明确。
...
| 是否识别开放阻断 | PASS；ISSUE-G3-01-001 已登记 |
