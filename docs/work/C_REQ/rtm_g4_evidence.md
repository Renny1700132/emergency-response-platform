...
| Task | 需求/指标与设计挂接 | 实现与自检证据 | Review / Merge 证据 | 追踪结论 |
| --- | --- | --- | --- | --- |
| G4-05 | G2-FR-013（AC-01、03）；G2-FR-014（AC-01、02）；G2-FR-015（AC-01、02）；G2-FR-016（AC-01）；G2-FR-022（AC-01、02）；G2-FR-028（AC-01）；MOD-EVENT/TASK/PLATFORM；ARCH-02；DLD/DBD/API-TR-013/014/015/016/022/028；ENG-015/017 | `backend/src/database.mjs`、`event-workflow.mjs`、`server.mjs`、`event-persistence.mjs`、`idempotency.mjs`、`identity.mjs`、`middle-platform-port.mjs`、`message-port.mjs`；`backend/migrations/002_event_workflow.*.sql`；`tests/backend/event-workflow.test.mjs`、`g4-05-integration.test.mjs`、`g4-05-recovery.test.mjs`、`g4-05-postgres.integration.mjs`、`foundation.test.mjs`；`quality/selfcheck/G4-05.json`；A 完整 `npm run quality` PASS；隔离 PostgreSQL 迁移、跨实例恢复与回滚测试 2/2 PASS | 原功能分支 `codex/g4-05-core-event-backend`、Gitee PR `!13` 已合并；G4-07 发现 `ISSUE-G4-07-002/003` 后由 B 在 `codex/g4-05-integration-remediation` 通过 PR `!14` 补齐事件/任务查询、临时任务、催办、文件预签名、Bearer 中台身份适配与页面上下文路由；证据 `logs/reviews/2026-09-25_G4-05-B-integration-remediation.md`；待 A Review 与 C 非 Mock E2E 复验 | IMPLEMENTED / PR_14_OPEN / G4-07_RETEST_PENDING；冻结契约、数据库恢复、事务原子性、正常、无权、幂等冲突、页面必需路由、Bearer 授权、消息失败/超时和人工降级均有自动化覆盖。消息和中台 Fixture 为 `SIMULATED_EVIDENCE`；真实甲方中台、文件服务及 KN-011/012/006/007 目标环境实测不冒充通过。 |
| G4-06 | PARTIAL：G2-FR-013/014/015/021/022；MOD-EVENT/TASK/MOBILE；DLD-TR-013/014/015/021/022。已实现前端：上报/核实/启动、任务接收/反馈/完成/催办/临时任务、H5 预签名上传失败重试。DEFERRED：FR-013 组合检索（冻结列表契约无筛选参数，需 B 受控契约变更后进入 G4-09）；真实后端/宿主相机/中台文件/消息/审计/性能 E2E（G4-07） | 整改提交 `0d537b9`；DoD 修正提交 `e951cba`；`IncidentFlowView.vue`、`TaskFlowView.vue`、`PlatformAttachmentField.vue`、`workflow.ts`；`page-interactions.test.ts` 挂载真实 Vue 页面/组件，覆盖 Web/H5 路由、loading/empty/403/failure/traceId、权限按钮、表单写操作与刷新、上传失败重试；`workflow.test.ts` 覆盖类型化请求构造；`quality/selfcheck/G4-06.json`。C 最终复跑 quality：frontend 27/27、覆盖率 95.41/76.19/95.12/100%；不声明整项 FR/DLD 或 E2E 已完成 | Gitee PR `!10`；唯一审核人 C（`rjq010504`）；最终复验证据 `logs/reviews/2026-09-24_G4-06-C-final-review.md`；`ISSUE-G4-06-001/002` 均已关闭；待平台 APPROVE 与 Merge | DONE CANDIDATE / C REVIEW PASS；G4-06 组件/交互门禁通过，真实前后端 E2E 仍由 G4-07 准出；PR 合并进入 master 后正式完成。 |
| G4-07 | G4-05/G4-06 Sprint 1 集成准出；G2-FR-013/014/015/016/021/022；KN-006/007/011/012/045；ENG-015/017 | 基线 `npm run quality` PASS：frontend 27/27、G4 11/11、backend 14/14；后端覆盖率 87.38/85.76/80.95/87.38%；OpenAPI 0 error；漏洞 0。真实 E2E 尚未通过：后端缺少前端必需查询/部分命令路由，Bearer 与后端身份解析未接通；本机无 Docker、无 `DATABASE_URL`，PostgreSQL 独立复跑待可用环境 | `ISSUE-G4-07-001` 已关闭；`ISSUE-G4-07-002/003` OPEN；预检报告 `logs/reviews/2026-09-25_G4-07-integration-precheck.md`；尚未提交 Review/Approve/Merge | BLOCKED / CHANGES_REQUIRED；不得将各自单测、mock 页面测试或 G4-05 既有 PostgreSQL 证据冒充 G4-07 独立真实前后端 E2E。 |
