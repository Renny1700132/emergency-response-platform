...
import OperationsModuleView from '@/views/OperationsModuleView.vue'
import { moduleRoutes } from './modules'

const children = (audience: 'web' | 'h5'): RouteRecordRaw[] => moduleRoutes.filter((item) => item.audience === audience).map((item) => ({
  path: item.path,
  component: item.id === 'MOD-EVENT' ? IncidentFlowView : item.id === 'MOD-TASK' ? TaskFlowView : OperationsModuleView,
...
export function createAppRouter(auth: AuthContext) {
  const router = createRouter({ history: createWebHistory(import.meta.env.BASE_URL), routes: [
    { path: '/', redirect: '/web/overview' },
    { path: '/web', component: WebLayout, children: [{ path: 'overview', component: DashboardView, meta: { title: '应急态势总览', requiresAuth: true, permissions: ['situation:read'] } }, ...children('web')] },
    ...
    { path: '/:pathMatch(.*)*', redirect: '/web/overview' },
  ] })
