...
8. 历史例外：G4-03 的实现 PR `!8` 与 G4-04 的实现 PR `!3` 均在 2026-09-24 单 PR 修订生效前合并，但未按当时流程完整回填 DONE/RTM；经用户明确授权，各允许一个仅含治理与证据回填的历史收口 PR。原任务主责/唯一审核关系不变，收口 PR 不修改业务实现；该例外不得复制到 G4-05 及后续任务。
