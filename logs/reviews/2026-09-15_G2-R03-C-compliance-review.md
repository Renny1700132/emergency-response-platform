...
- 复验提交：`c159709`（A 修订），远程复验基点 `098e488`。
- 复验材料：修订后的 Markdown、正式 Word、构建脚本，以及 `logs/reviews/render_G2-R03_SRS_review_fix/page-1.png`—`page-18.png`。
- 已通过：教学样例目录条目已清除；第5章调整为 `5.1→5.2→5.3→5.4`；Word 表5-1恢复类型/必填维度并与 Markdown 一致；旧 G2-04/G2-05/G2-06 引用已修正；39 FR、34★、PE-01—12及10条 G2-RCLR 未发生漂移；全页未见截断、重叠或溢出。
- 仍有 ISSUE：正式 Word 渲染第12张（正文页10）先出现 `4 外部接口需求`，其后才出现 `3.6 核心业务流程与异常路径` 和 `3.7 核心状态模型`，第13张随后进入表4-1。实际正文顺序为 `3.5→4→3.6→3.7→4章内容→5`，与目录及 Markdown 的 `3.5→3.6→3.7→4→5` 不一致。构建脚本将3.6/3.7插入在第4章标题锚点之后，是该问题的可复核来源。
- 复验结论：`ISSUE-G2-R03-001` 不满足关闭判据，继续保持 `OPEN / BLOCKING`；G2-R03 **NOT ACCEPTED**，继续保持 `REVIEW`。A 修正章节插入顺序并重新渲染后，须由 C 再次复验；B 技术复核仍待独立完成。

## 7 二次修订后复验（2026-09-15）

- 复验提交：`f801615`（A 二次修订），远程复验基点 `1e6f60b`。
- 结构与内容：`scripts/check_g2_r03_review.py` 返回 PASS；正式 Word 的 OOXML 正文顺序已为 `3.5→3.6→表3-6→3.7→表3-7→4→5`，与静态目录和 Markdown 一致。DOCX ZIP 完整性通过，39个唯一 G2-FR、34条★、PE-01—12、G2-RCLR-001—010、17张表及 A4 页面参数未漂移。C 确认上一轮唯一语义/结构阻断已经修正。
- 渲染门禁：C 使用 documents Skill 的标准 `render_docx.py` 对二次修订正式件重新渲染，因受控运行环境未提供 `soffice.exe` 而失败，未生成本次版本的新全页 PNG；仓库中的18页渲染属于前一版本，不能替代二次修订后的逐页视觉证据。该失败与 A 自检记录一致，未写成视觉通过。
- C 复验结论：内容、数字、★、责任边界、Markdown/Word 结构一致性 **PASS**；正式 Word 最新版本的全页视觉检查 **BLOCKED / 待可用渲染环境**。因此 `ISSUE-G2-R03-001` 暂不关闭，G2-R03 继续保持 `REVIEW`；取得最新版本全页渲染并由 C 检查无缺陷、且 B 完成技术复核后方可准出。

## 8 WPS 全页视觉复验（2026-09-15）

- 渲染对象：提交 `f801615` 对应正式件 `docs/deliverables/08-软件需求规格说明书SRS.docx`，SHA-256 `E370C7A0EAC103FBAA91ECE6F8AFE6321BFFC3B12A0EDC26C7576FD05521CC15`。
- 方法与证据：使用 WPS Office 12.1.0.28505 成功打开正式 DOCX 并输出18页 PDF；再用受控 Poppler 以144 DPI生成 `logs/reviews/render_G2-R03_SRS_WPS_review2/page-01.png`—`page-18.png`，C 对18/18页逐页目视检查。
- PASS：正式件可由 WPS 正常打开和导出；第12—13页确认3.6、表3-6、3.7、表3-7均位于第4章之前，章节顺序阻断已修复；正文、表格无裁切、重叠、乱码或越界。
- ISSUE / MAJOR：第3页在居中显示3.6—10目录后，又在左侧重复显示同一组3.6—10目录，随后才进入第1章，目录内容明显重复。
- ISSUE / MAJOR：第14页表5-1结束后，在5.3标题前再次出现“表 5-1 核心实体字段字典”，形成重复题注。
- ISSUE / MINOR：第4页图1-1各框内文字为近白色浅字，和浅色底纹对比度不足，投影/打印场景下可读性较差；建议 A 提高文字对比度并重渲染。
- 结论：WPS 渲染门禁已执行，但发现上述可见缺陷；`ISSUE-G2-R03-001` 继续 `OPEN / BLOCKING`，G2-R03 保持 `REVIEW`。A 修复重复目录、重复题注及图中文字对比度后，应以 WPS 重新输出并由 C 再次逐页复验；B 技术复核仍须独立完成。
