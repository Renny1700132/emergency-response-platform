...
每次任务的用户 Prompt 原文必须在实质执行前逐字写入 `logs/prompts/YYYY-MM-DD.md` 的 `USER_PROMPT_RAW`。本目录可保存可复用模板或额外 Prompt 文件，但不能代替当日日志中的原文。摘要或人工重写不得标为“原始 Prompt”。无法导出完整用户可见对话时写 `raw_dialogue_available: false`；这不免除保存本次用户 Prompt 原文和最终用户可见输出原文的义务。
