...
const seed=()=>({
  events:[
    {id:'EVT-20260914-001',title:'东区展厅烟感告警',type:'消防告警',location:'东区展厅',description:'模拟烟感设备产生告警，等待人工核实。',status:'待核实',planId:null,createdBy:'模拟物联网',updatedAt:'16:20',timeline:[{time:'16:20',label:'事件上报',detail:'模拟物联网告警转入待核实事件'}]},
    {id:'EVT-20260914-002',title:'一层客流密度异常',type:'客流异常',location:'一层中庭',description:'模拟客流密度超过展示阈值。',status:'处置中',planId:'PLAN-CROWD-001',createdBy:'模拟客流平台',updatedAt:'16:05',timeline:[{time:'15:54',label:'事件上报',detail:'客流平台模拟告警'},{time:'15:57',label:'核实通过',detail:'值班人员确认需要处置'},{time:'16:05',label:'启动预案',detail:'关联客流疏导预案'}]}
  ],
  tasks:[
...
const load=()=>{
  ...
  try{return JSON.parse(window.localStorage.getItem(storageKey))||seed()}catch{return seed()}
}
...
export const actions={
  ...
  reportIncident(input){
    ...
    const event={id:nextId('EVT'),title:input.title.trim(),type:input.type,location:input.location.trim(),description:input.description?.trim()||'未填写补充说明',status:'待核实',planId:null,createdBy:'H5 值班人员',updatedAt:now(),timeline:[{time:now(),label:'H5 事件上报',detail:'已保存事件信息；图片为 Mock 引用'}]}
  ...
  closeIncident(id){
    ...
    event.status='已关闭';addTimeline(event,'事件关闭','已形成模拟评估与调查归档入口');persist();return {ok:true}
  },
  advanceDrill(result='按计划完成疏散集合'){
    const order=['待执行','执行中','待评估','已完成'],index=order.indexOf(state.drill.status)
    if(index<0||index===order.length-1)return {ok:false,message:'演练已经完成'}
    state.drill.status=order[index+1]
    if(state.drill.status==='待评估')state.drill.result=result
    if(state.drill.status==='已完成')state.drill.improvement='加强疏散口引导标识检查'
    persist();return {ok:true}
  },
  checkin(valid=true){
    const record={id:nextId('CHK'),person:'当前值班人员',point:'东区展厅入口',time:now(),status:valid?'有效':'已拒绝',reason:valid?'身份、时段和模拟范围校验通过':'模拟超出有效范围'}
    state.checkins.unshift(record);persist();return {ok:valid,record,message:record.reason}
  },
  submitInventory(value){
    const actual=Number(value)
    if(!Number.isInteger(actual)||actual<0)return {ok:false,message:'盘点数量必须是非负整数'}
    state.inventory.actualQuantity=actual;state.inventory.difference=actual-state.inventory.bookQuantity;state.inventory.status='待复核';persist();return {ok:true}
  },
  reviewInventory(){
    if(state.inventory.status!=='待复核')return {ok:false,message:'仅待复核结果可确认更新'}
    state.inventory.bookQuantity=state.inventory.actualQuantity;state.inventory.status='已复核';state.inventory.reviewer='物资管理员';persist();return {ok:true}
  }
}
