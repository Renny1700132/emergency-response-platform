...
| G3-06 | 接口设计与 OpenAPI | B | A、C | G3-03、G3-05 | 模块边界、外部责任边界、数据设计 | 接口设计说明书、OpenAPI 契约与版本策略 | REVIEW（B 已按 `ISSUE-G3-06-001` 完成 V0.2 整改：补充 PPT 可编辑接口交互图，修复表 2-1/7-1 跨渲染版式并通过 WPS/Word 双渲染；待 A/C 复核关闭该 Issue。`ISSUE-G3-01-001` 仍阻断视频/消息真实外部契约及最终冻结） |
