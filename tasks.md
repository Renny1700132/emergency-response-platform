...
| G4-04 | Sprint 1 | 八外部端口、GIS/H5 的课程模拟服务与测试 Fixture | C | B | G4-00；G4-01；冻结 OpenAPI 与模拟确认 | 8 个独立外部端口、GIS/H5 模拟服务，正常/无权/超时/失败 Fixture 与使用说明 | EXT-VIDEO/PUBLISH/INTRUSION/ACCESS/FIRE/IOT/MIDDLE/MESSAGE 逐一可辨识；GIS/H5 边界明确；Fixture 可重复、无秘密；所有证据标记 SIMULATED；契约测试通过 | REVIEW |
