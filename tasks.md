...
当前进展：第一关 G1-00 至 G1-14 已完成并冻结为 BASELINE-V1.0。第二关输入资料已归档；G2-00 因新规格说明书与第一关冻结项目不一致，等待用户确认项目身份与 MVP 继承关系。
...
| Task ID | 任务 | 主责 | 复核 | 前置任务 | 输入 | 输出 | 状态 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| G2-00 | 输入归档与范围决策 | C | B | 无 | 第二关三份输入资料、BASELINE-V1.0 | 输入索引、范围决策记录、基线启动条件 | BLOCKED |
| G2-01 | 用户访谈与 MoSCoW | A | C | G2-00 | 确认的项目范围、MVP 输入 | 访谈记录、用户故事与 MoSCoW | TODO |
| G2-02 | 需求解析与编号基线 | C | B | G2-00 | 确认的项目输入、访谈结论 | facts、术语、编号目录、关键数字 | TODO |
| G2-03 | SRS v0.1 | A | C | G2-01、G2-02 | 用户故事、需求基线 | SRS 工作稿 | TODO |
| G2-04 | spec.md v0.1 | B | A | G2-02 | 需求编号、验收约束 | spec.md | TODO |
