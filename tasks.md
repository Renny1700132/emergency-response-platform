...
| G3-09 | 测试计划 | C | A（本轮按 OVR-025 单人复核） | G3-03—G3-08 | spec AC、RTM、设计文档集、GB/T 15532 | 《测试计划》与测试左移准入/准出依据 | REVIEW（2026-09-19 C 已完成 A 审核整改：核验 G3-08 当前为 DONE；工作稿/正式件复核主体统一为 A；目录补齐页码、点引线和二级层级；表 8-1 改为可读四列左对齐；图 3-1 已加替代文本。`ISSUE-G3-09-001/002` 均为 RESOLVED_BY_C / PENDING_A_REVERIFY，待 A 单人复验后方可 DONE。尚未执行测试；`ISSUE-G3-01-001` 保持 OPEN） |
