...
test('HTTP core incident path covers authorization, idempotency, tasks and closure', async (t) => {
  ...
  assert.equal((await post(server, `/api/v1/incidents/${incidentId}/verify`, { decision: 'VERIFIED', reason: 'confirmed' })).status, 200);
  const started = await post(server, `/api/v1/incidents/${incidentId}/start-response`, { planVersionId: 'plan-v1' });
  assert.equal(started.body.data.tasks[0].deliveryStatus, 'ACCEPTED');
  const taskId = started.body.data.tasks[0].id;
  assert.equal((await post(server, `/api/v1/tasks/${taskId}/acknowledge`, { reason: 'received' })).status, 200);
  ...
  assert.equal((await post(server, `/api/v1/tasks/${taskId}/complete`, { reason: 'done' })).status, 200);
  const closed = await post(server, `/api/v1/incidents/${incidentId}/close`, { reason: 'safe', attributes: { reportRef: 'report-1' } });
